...
void maxHeap1::buildQueue( string fileName){
...
void maxHeap1::buildHeap(int** heapArray, int numPatients, int i){
...
bool maxHeap1::hasPriority(int** heapArray, int x, int y){
	bool priority = false;
...
void maxHeap1::queuePatient( int* patient){
	numPatients++;
...
bool maxHeap1::hasArrived(int* patient,int time){
	return (patient[2] <= time);
}
//calculates average waiting time for given number of doctors
double maxHeap1::calculateTime( int numDoctors){
...
double maxHeap1::simulate( int numDoctors){
	int* doctorArray = new int[numDoctors]; //this array holds remaining examination time for doctors
	int** tempArray = new int*[numPatients]; // this will be used to put the patients who didn't arrive yet (temporarily)
	int numTemp = 0; //number of patients in tempArray
	int numPatients2 = numPatients; // needed to calculate avg. time since numPatients become 0 at the end
...
void maxHeap1::printHeap(int** arr, int n)
{
    for (int i = 0; i < n; ++i){
		cout << arr[i][0] << " ";
        cout << arr[i][1] << " ";
